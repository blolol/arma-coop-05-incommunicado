/*
			...
			Calling the script:
					...
					default: nul = [this] execVM "vendor\lv\fillHouse.sqf";
					
					custom:  nul = [target, side, patrol, patrol type, spawn rate, radius, skills, group, 
									custom init, ID] execVM "vendor\lv\fillHouse.sqf";
...
EXAMPLE: 	nul = [this, 2, true, 2, 50, 1, 0.75, nil, nil, 9] execVM "vendor\lv\fillHouse.sqf";
...
if(isNil("LV_nearestBuilding"))then{LV_nearestBuilding = compile preprocessFile "vendor\lv\LV_functions\LV_fnc_nearestBuilding.sqf";};
...
while{_i2 < _rat}do{
	...
	if(_patrol)then{
        	switch (_pType) do {
            		case 1: {
                		_pFile = "vendor\lv\patrol-vF.sqf";
            		};
            		case 2: {
                		_pFile = "vendor\lv\patrol-vG.sqf";
            		};
        	};
